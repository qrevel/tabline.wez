...
local function process_to_icon(colors)
  return {
    ...
    ['git'] = { wezterm.nerdfonts.dev_git, color = { fg = colors.ansi[2] or nil } },
...
return {
  ...
  update = function(tab, opts)
    ...
    local process_to_icon = type(opts.process_to_icon) == 'function'
      and opts.process_to_icon(require('tabline.config').theme.colors)
      or opts.process_to_icon
    if opts.icons_enabled and process_to_icon then
      for process, _ in pairs(process_to_icon) do
        if foreground_process_name:lower():match('^' .. process) then
          util.overwrite_icon(opts, process_to_icon[process])
          icon_set = true
          break
        end
      end
    ...
    if not icon_set then
      util.overwrite_icon(opts, process_to_icon['default'])
    end
